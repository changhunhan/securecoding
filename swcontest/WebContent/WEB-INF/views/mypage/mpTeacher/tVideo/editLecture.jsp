...
<html>
<head>
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/css/editLecture.css">
...
<script>
	...
	var checkInitBookMark = true;
	$(function() {
		$("#lecture").on("timeupdate", function() {
			$("#currentTime").val(this.currentTime);
		})
		if("${BMList.size()}" != 0) {
			<c:forEach items="${BMList}" var="marker">
			timeMap.set(count, "${marker.bkmark_time}");
			textMap.set(count, "${marker.bkmark_title}");
			contMap.set(count, "${marker.bkmark_cont}");
			count++;
			</c:forEach>
		}
		
		$("#lec_cont").append("${lecVo.lec_cont}");
		...
		$(document).on("change", "#video", function() {
			if(timeMap.size != 0 && checkInitBookMark) {
				player.markers({
					   markers: [
							<c:forEach var="i" begin="0" end="${size}">
							 	{time : parseInt(timeMap.get(${i})), text : textMap.get(${i})},
							</c:forEach>    
					   ],
					   onMarkerClick: function(marker) {
						   $("#bookMarkText").val(textMap.get(getTimeKey(parseInt(marker.time))));
			...
			if(videoName == undefined) {
				...
				var ext = videoName.name.substring(dot).toLowerCase();
		...
		$(document).on("click", "#register", function() {
			var formData = new FormData(document.getElementById("editVideo"));
			...
			$.ajax({
				url: "${pageContext.request.contextPath}/video/editUpload",
				type: "POST",
				data : formData,
				async : false,
				cache : false,
	...
	<div class="content-wrapper" id="conWBground">
		...
		<div class="row">
			...
			<section class="col-lg-8"> 
				...
				<form id="editVideo" method="post" enctype="multipart/form-data">
						<div class="row" style="margin-top:10px">
							<div class="col-xs-4" style="margin-left: 10px">
								<label for="id">동영상 수정</label> <input type="text"	id="videoName" name="vio_name" value="${videoVo.vio_name}" class="form-control input-sm" placeholder="Video Upload" readonly>
							</div>
							<div class="form-group" style="margin-top: 22.5px">
								<input type="file" id="video" name="video" accept="video/*">
								<button type="button" id="videoUpload"
								class="btn btn-primary" style="background-color:#3c8dbc; border:none">
								<b>동영상 수정</b>
							</button>
						...
						<div class="row" style="margin:10px;">
							<div class="col-lg-8">
						        <video id="lecture" class="video-js vjs-default-skin"
						               ...
						               data-setup='{"controls":true, "playbackRates": [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]}' style="width:99.98%;">
						       </video>
						    </div>
						    <input type="hidden" id="tol_vio_time" name="tol_vio_time">
						    <input type="hidden" id="vio_size" name="vio_size" value="0">
						    <input type="hidden" id="vio_num" name="vio_num" value="${videoVo.vio_num}">
						    <input type="hidden" id="lec_num" name="lec_num" value="${lecVo.lec_num}">
						    <div class="col-lg-4">
						...
						<div class="form-group has-feedback" id="posterDiv"
							style="margin: 10px; margin-top: 0px">
							<img src="http://placehold.it/150x100" style="width:150px; height:100px;" id="posterImage" alt="poster" class="margin">
							<input type="file" id="poster" name="poster" accept="image/*">
								<button type="button" id="posterUpload"
								class="btn btn-primary" style="background-color:#3c8dbc; border:none">
								<b>동영상 포스터 등록</b></button>
						</div>
						<div class="form-group has-feedback"
							style="margin: 10px; margin-top: 0px">
							<label for="id">강의 제목</label> <input type="text" name="lec_title" value="${lecVo.lec_title}" class="form-control input-sm" placeholder="Video Title"> <span
						...
						<div class="form-group has-feedback" style="margin: 10px">
							...
							<textarea class="textarea" id="lec_cont" name="lec_cont" placeholder="Place some text here" style="width: 100%; height: 75px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"></textarea>
						</div>
						<div class="row">
							<!-- /.col -->
							<div class="col-xs-2" id="dd">
								<button type="button" id="register" onclick="duration()" class="btn btn-primary btn-block btn-flat" style="background-color:#3c8dbc; border:none">저장</button>
							</div>
							<!-- /.col -->
						</div>
						</form>
