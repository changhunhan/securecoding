...
public class tStudentController {
	...
	@Resource(name="tStudentService")
	private tStudentService service;
	
	@Autowired
	private DDayCount dday;
	
	@RequestMapping(value="/tStudent/tStudentMain", method=RequestMethod.GET)
	public String returnForm(HttpSession session,Model model){
		...
		List<CurriReqVo> list=service.curriReqMem(id);
		
		model.addAttribute("curriReq",list);
